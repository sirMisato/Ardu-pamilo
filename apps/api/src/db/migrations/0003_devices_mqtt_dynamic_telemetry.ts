...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP TABLE IF EXISTS telemetry_metric_readings`.execute(db);
  await sql`DROP TABLE IF EXISTS telemetry_metric_catalog`.execute(db);
  await sql`DROP TABLE IF EXISTS device_mqtt_credentials`.execute(db);
